...
def analyze_text(text, keywords):
    ...
    sorted_dict_desc = dict(sorted(keyword_count.items(), key=lambda item: item[1], reverse=True))
    word_count = Counter(words)
    return sorted_dict_desc, word_count
...
def main():
    ...
    keywords = ['python', 'R', 'Alteryx', 'SQL', 'MySQL', 'SQLite', 'SAS', 'Stata', 'matlab',
                'machine learning', 'data analysis', 'data science', 'model'
                'dplyr', 'pandas', 'numpy', 'ggplot', 'matplotlib'
                ]   # Add defined list of keywords here
    common_words = 15
